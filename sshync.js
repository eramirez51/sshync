...
const SETTINGS_FILE = '.sshync.json';
...
function showHelp() {
  console.log(
    `${chalk.bold('sshync')} - Auto-sync files or directories over SSH\n\n` +
    `${chalk.yellow('Usage:')}\n` +
    `  sshync <${chalk.blue('source')}> <user@ip[:port]:${chalk.green('destination')}> [--ignore ${chalk.magenta('folders')}]\n` +
    `  sshync init <${chalk.blue('source')}> <user@ip[:port]:${chalk.green('destination')}> [${chalk.cyan('filename')}] [--ignore ${chalk.magenta('folders')}]\n` +
    `  sshync load [${chalk.cyan('filename')}]\n\n` +
    ...
    `  sshync init ./src user@server:/app dev.json --ignore node_modules,tmp`
  );
}
...
function initSettings(source, destination, filename = SETTINGS_FILE, ignoreFolders = []) {
  ...
  try {
    ...
    console.log(chalk.gray(`  Run 'sshync load${filename !== SETTINGS_FILE ? ` ${filename}` : ''}' to start syncing`));
  } catch (error) {
...
function loadSettings(filename = SETTINGS_FILE) {
  try {
    if (!fs.existsSync(filename)) {
      ...
      console.log(chalk.gray(`  Run 'sshync init <source> <destination>${filename !== SETTINGS_FILE ? ` ${filename}` : ''}' first`));
      process.exit(1);
    }
...
function startSync(sourcePath, destinationPath, ignoreFolders = []) {
  ...
  // Add .sshyncignore file if it exists
